...
class VoidReturnAdder:
    @staticmethod
    def addReturn(cfg: map):
        for key, value in cfg.items():

            if value.accepting.node_link is not None and value.accepting.node_link.getChildAmount() == 0 or value.accepting.node_link is not None and value.accepting.node_link.getChild(
                    -1).text != 'Return':
