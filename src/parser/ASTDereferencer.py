...
class ASTDereferencer(ASTVisitor):
    ...
    def visitNode(self, node: ASTNode):
        ...
        if left_child.text == "*":
            # if right_child.text == "Dereference" and right_child.children[0].text == "Expr" and right_child.children[0].children[1].text == "[]":  # Special case when trying to acces a struct data member, an extra dereference should be prevented
            #     node.parent.replaceChild(node, right_child)
            #     # TODO Should this hold in general? Or for arrays too?
            # else:
    ...
    def isStructPtr(node):
        ...
        if isinstance(type_object, SymbolTypePtr):
            ...
            return isinstance(type_object, SymbolTypeStruct)
